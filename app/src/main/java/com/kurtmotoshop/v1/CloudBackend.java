...
public class CloudBackend {
    ...
    private void reconcileCollectionSync(String collection, JSONArray array) throws Exception {
        ...
        // Anything belonging to this shop that was not uploaded anymore is
        // a local deletion. Mark it hidden instead of physically deleting it.
    ...
    public void writeTransactionState(String stateJson, Callback<Boolean> cb) {
        new Thread(() -> {
            try {
                requireAuth();
                JSONObject state = new JSONObject(stateJson);
                writeCollectionSync("sales", state.optJSONArray("salesHistory"));
                writeCollectionSync("stockHistory", state.optJSONArray("stockHistory"));
                cb.onResult(true, null);
            } catch (Exception e) {
                cb.onResult(false, e);
            }
